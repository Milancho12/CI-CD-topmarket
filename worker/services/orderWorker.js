...
process.on('message', async ({ account, date }) => {
  ...
  try {
    ...
    if (!page.url().includes('Items_x_Clients.aspx')) {
      await page.goto('http://zitoluks.ddns.net/MatrixTables/Items_x_Clients.aspx', { waitUntil: 'domcontentloaded' });
    }
    ...
    for (const task of account.driverTasks) {
      ...
      await page.evaluate((payload, colIdx) => {
        ...
        // 1. Clear all inputs in this column
        ...
        // 2. Set quantities for active items
    ...
    } catch (e) { /* page navigated or detached */ }
